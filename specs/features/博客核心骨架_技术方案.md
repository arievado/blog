...
export const siteConfig = {
  ...
  socialLinks: [                   // 社交链接，在首页 Sidebar 展示
    ...
    { name: "B站", url: "https://space.bilibili.com/xxxxx", icon: "bilibili" },
    { name: "QQ音乐", url: "https://y.qq.com/", icon: "qqmusic" },
    { name: "邮箱", url: "mailto:contact@example.com", icon: "email" },
  ],
} as const;
```
...
- `name`：链接显示名称
- `url`：链接地址，点击后新标签页打开
- `icon`：图标标识符，由 Sidebar 组件内 `iconMap` 字典映射到 SVG（支持 `github`、`bilibili`、`qqmusic`、`email` 等，新增平台只需在字典中追加一条 key → SVG 映射）
...
- 图标大小 20x20，hover 变色
- icon → SVG 映射使用 `iconMap` 字典（`Record<string, string>`），通过 `link.icon` 做 key 查找，告别 if/else 链
- 支持 icon：`github`、`bilibili`、`qqmusic`、`email` 等；新增平台只需在字典中追加一个条目
...
| 2026-05-11 | CR-007：3.3 节 socialLinks 示例扩展、5.6 节 b) icon 映射改为字典结构 | 新增 B站/QQ音乐/邮箱社交图标，icon 渲染从 if/else 链重构为字典查找 |
